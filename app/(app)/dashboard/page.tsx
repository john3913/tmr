  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((card) => {
          ...
          return (
            <div
              key={card.label}
              className={`bg-white rounded-xl border border-[#dce8f5] border-t-2 ${card.accent} p-5`}
            >
              <div className="flex items-start justify-between">
                ...
                {card.positive !== null && (
                  <ArrowUpRight className={`w-4 h-4 ${card.positive ? "text-[#1a6b3c]" : "text-red-400"}`} />
                )}
              </div>
              <p className="text-2xl font-bold text-slate-900 mt-3">{card.value}</p>
              <p className="text-xs text-slate-500 mt-0.5">{card.label}</p>
              <p className={`text-xs mt-1.5 font-medium ${
                card.positive ? "text-[#1a6b3c]" : card.positive === false ? "text-red-500" : "text-slate-500"
              }`}>
                {card.change}
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl border border-[#dce8f5] p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-slate-900">Compliance Health</h2>
            <TrendingUp className="w-4 h-4 text-[#0079BE]" />
          </div>
          <div className="flex flex-col items-center py-2">
            <div className="relative w-32 h-32">
              <svg viewBox="0 0 120 120" className="w-full h-full -rotate-90">
                <circle cx="60" cy="60" r="50" fill="none" stroke="#e0eaf6" strokeWidth="10" />
                <circle
            ...
            <div className="mt-5 w-full space-y-2.5">
              {[
                { label: "ERISA", pct: 98, color: "#1a6b3c"  },
                { label: "ACA",   pct: 91, color: "#0079BE"  },
                { label: "COBRA", pct: 82, color: "#f59e0b"  },
                { label: "FMLA",  pct: 97, color: "#1a6b3c"  },
              ].map((item) => (
                <div key={item.label} className="flex items-center gap-3">
                  <span className="text-xs text-slate-500 w-12">{item.label}</span>
                  <div className="flex-1 h-1.5 bg-[#e0eaf6] rounded-full overflow-hidden">
                    <div
        ...
        <div className="bg-white rounded-xl border border-[#dce8f5] p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="text-xs text-[#0079BE] hover:underline font-medium">View all</button>
          </div>
        ...
        <div className="bg-white rounded-xl border border-[#dce8f5] p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <CalendarRange className="w-4 h-4 text-[#0079BE]" />
          </div>
          <div className="space-y-3">
            {deadlines.map((d, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-[#f0f6fd] border border-[#dce8f5]">
                ...
                <ChevronRight className="w-3.5 h-3.5 text-[#0079BE]/40 mt-0.5 flex-shrink-0" />
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Plans Table */}
      <div className="bg-white rounded-xl border border-[#dce8f5] overflow-hidden">
        <div className="px-5 py-4 border-b border-[#dce8f5] flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-900">Benefit Plans — Enrollment & Status</h2>
          <div className="flex items-center gap-2">
            <FileText className="w-4 h-4 text-[#0079BE]/50" />
            <button className="text-xs text-[#0079BE] hover:underline font-medium">Export</button>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#f0f6fd]">
                <th className="text-left px-5 py-3 text-xs font-semibold text-[#005294] uppercase tracking-wide">Plan</th>
                <th className="text-right px-5 py-3 text-xs font-semibold text-[#005294] uppercase tracking-wide">Enrolled</th>
                <th className="text-right px-5 py-3 text-xs font-semibold text-[#005294] uppercase tracking-wide">Eligible</th>
                <th className="text-right px-5 py-3 text-xs font-semibold text-[#005294] uppercase tracking-wide">Rate</th>
                <th className="text-center px-5 py-3 text-xs font-semibold text-[#005294] uppercase tracking-wide">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#e8f0fb]">
              {plans.map((plan) => {
                ...
                return (
                  <tr key={plan.name} className="hover:bg-[#f0f6fd] transition-colors">
                    ...
                    <td className="px-5 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <div className="w-20 h-1.5 bg-[#e0eaf6] rounded-full overflow-hidden">
