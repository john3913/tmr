"use client";
import Link from "next/link";
import { usePathname } from "next/navigation";
import {
  LayoutDashboard,
  ShieldCheck,
  CalendarRange,
  Users,
  Building2,
  FileBarChart2,
  Settings,
} from "lucide-react";

const nav = [
  { href: "/dashboard", label: "Dashboard", icon: LayoutDashboard },
  { href: "/audit", label: "Benefits Audit", icon: ShieldCheck },
  { href: "/enrollment", label: "Open Enrollment", icon: CalendarRange },
  { href: "/employees", label: "Employees", icon: Users },
  { href: "/vendors", label: "Vendors & Carriers", icon: Building2 },
  { href: "/reports", label: "Reports & Filings", icon: FileBarChart2 },
];

export default function Sidebar() {
  const pathname = usePathname();
  return (
    <aside className="w-60 flex-shrink-0 bg-[#f8fbff] border-r border-[#dce8f5] flex flex-col h-screen sticky top-0">
      {/* Logo */}
      <div className="px-5 py-5 border-b border-[#dce8f5]">
        <Link href="/" className="block hover:opacity-75 transition-opacity">
          <p className="text-sm font-bold text-slate-900 leading-tight">TMR Benefits</p>
          <p className="text-[10px] text-[#0079BE] leading-tight uppercase tracking-wide font-mono">ALLETE Compliance</p>
        </Link>
      </div>

      {/* Nav */}
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {nav.map(({ href, label, icon: Icon }) => {
          const active = pathname === href || pathname.startsWith(href + "/");
          return (
            <Link
              key={href}
              href={href}
              className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                active
                  ? "bg-[#0079BE]/10 text-[#0079BE]"
                  : "text-slate-600 hover:bg-[#0079BE]/5 hover:text-slate-900"
              }`}
            >
              <Icon className={`w-4 h-4 flex-shrink-0 ${active ? "text-[#0079BE]" : "text-slate-400"}`} />
              {label}
            </Link>
          );
        })}
      </nav>

      {/* Bottom */}
      <div className="px-3 py-4 border-t border-[#dce8f5] space-y-0.5">
        <Link
          href="/settings"
          className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors"
        >
          <Settings className="w-4 h-4 text-slate-400" />
          Settings
        </Link>
        <div className="px-3 pt-3">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-full bg-[#0079BE] flex items-center justify-center text-white text-xs font-bold">
              JD
            </div>
            <div className="min-w-0">
              <p className="text-xs font-medium text-slate-900 truncate">Jane Doe</p>
              <p className="text-[10px] text-slate-500 truncate">Benefits Admin</p>
            </div>
          </div>
        </div>
      </div>
    </aside>
  );
}
